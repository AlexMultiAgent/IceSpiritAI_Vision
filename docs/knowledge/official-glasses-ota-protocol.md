...
- **中途补救**（2026-09-17 用户反馈后补）：升级过程中若眼镜仍报「共享网络不可用」，
  每 20 s 重发一次 `0x3E`（用户往往是看到红字提示后才去打开手机开关）；一旦 `0x15` 变 1
  而当初下发 URL 时 PAN 还没就绪，就**再发一次 URL**（眼镜自己不会重试，只靠 App 重发）。
  对话框也允许随时关闭，行内继续显示「已等待 N 秒」并可点「查看」重新打开——此前关不掉，
  用户只能杀 App 重来。
