...
class AssetRuleLoaderTest {
    ...
    fun load_parsesActualBundledAdSignageAssetShape() {
        ...
        assertTrue(
            "shipped ad_signage_rules.json must bundle 10 golden + 70+ incremental rules (v1: 10, v2: +32, v3: +43 = 85, v4: +31 = 116 total)",
            set.rules.size >= 110,
        )
    ...
    fun load_parsesActualBundledFoodLabelAssetShape() {
        ...
        val set = json.decodeFromString(FoodLabelRuleSet.serializer(), src)
        assertEquals(3, set.version)
        assertTrue(
            "shipped food_label_rules.json must carry at least one rule",
            set.rules.size >= 1,
        )
        assertTrue(
            "shipped food_label_rules.json must bundle 6 golden + 30 incremental + 29 v3 rules",
            set.rules.size >= 60,
        )
        assertTrue(
