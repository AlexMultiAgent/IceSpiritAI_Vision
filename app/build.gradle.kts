...
android {
    ...
    // P1-H013 (2026-09-01 compatibility audit §3 H014 — APK 体积 ~80 MB):
    // enable AAB splits so future Play Store distribution can deliver
    // per-device-spec APKs (density / language / ABI). Current distribution
    // model is direct in-app update (single fat APK) per CLAUDE.md §发布
    // 流水线踩坑 — these settings do NOT alter the assembled direct APK,
    // they only take effect when the bundle is generated by
    // `bundleRelease` and uploaded to Play Store for dynamic delivery.
    //
    // Rationale per axis:
    //  - abi.enableSplit: arm64-v8a only (ndk.abiFilters above), so the
    //    single ABI split == the base APK; enableSplit stays true for
    //    forward-compat if a future product decision adds a second ABI.
    //  - density.enableSplit: app ships drawable-{m,h,xh,xxhdpi,xxxhdpi}
    //    densities; Play Store dynamic delivery drops non-matching ones.
    //    Saves ~5-15 MB on typical devices that match one density.
    //  - language.enableSplit: project ships only zh-CN string resources
    //    + a few universal `values/` fallbacks; splitting by language
    //    would let Play Store deliver a smaller base APK for non-zh-CN
    //    devices. Since the app is zh-CN-only at product level, the base
    //    APK without language splits already excludes non-matching locale
    //    resources (AAPT2 strips them); enableSplit=true is a no-op
    //    safety belt.
    bundle {
        abi {
            enableSplit = true
